...
3. Determine root cause (timing, selector, or implementation)
4. Use appropriate skill if available
5. Apply fix
6. Re-run tests
7. **ONLY delete the failure log if 100% of tests pass**: `rm .autoflow/.failures/sprint-{ID}-integration-tests.md`
...
- Search for most recent `*e2e-test-runner.log`
...
5. Apply fix
6. Re-run tests
7. Delete failure log when all pass
8. Output fix summary
